...
(function(){
  ...
  function printSheetHtml(doc){
    ...
    return `<div class="print-sheet apd-sheet"><style>
      ...
      .apd-photos{display:flex;flex-wrap:wrap;gap:12px}.apd-photo-item{width:190px}.apd-photo-item img{width:190px;height:142px;object-fit:cover;border-radius:6px;border:1px solid #ccd8e3;display:block}.apd-photo-item small{display:block;font-size:9.5px;color:#5b6f82;margin-top:3px;overflow-wrap:anywhere}
      @media print{.apd-sheet{padding:10mm 9mm;font-size:11px}.apd-card{break-inside:avoid}.apd-header{grid-template-columns:1fr 255px}.apd-asset-wrap{grid-template-columns:180px 1fr}.apd-asset-photo,.apd-photo-placeholder{width:180px;height:112px}.apd-photo-item{width:135px}.apd-photo-item img{width:135px;height:101px}}
    ...
    <section class="apd-card"><div class="apd-card-head"><span class="apd-icon">📄</span><span>คำขอ</span></div><div class="apd-card-body apd-grid"><div class="apd-field"><span class="apd-label">ผู้แจ้ง</span><span class="apd-value">${esc(r.requesterNameSnapshot||'-')}</span></div><div class="apd-field"><span class="apd-label">หน่วยงาน</span><span class="apd-value">${esc(a.unitName||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">อาการ / เหตุผล</span><span class="apd-value">${esc(r.issue||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">หมายเหตุ</span><span class="apd-value">${esc(r.requestNote||'-')}</span></div></div></section>
    <section class="apd-card"><div class="apd-card-head"><span class="apd-icon">🚙</span><span>ทรัพย์สิน</span></div><div class="apd-card-body apd-asset-wrap">${assetPhoto}<div class="apd-grid"><div class="apd-field"><span class="apd-label">รหัส</span><span class="apd-value">${esc(a.code||'-')}</span></div><div class="apd-field"><span class="apd-label">ทะเบียน / ชื่อ</span><span class="apd-value"><b>${esc(a.plate||'-')}</b></span></div><div class="apd-field"><span class="apd-label">ยี่ห้อ / รุ่น</span><span class="apd-value">${esc(`${a.brandName||''} ${a.modelName||''}`.trim()||'-')}</span></div><div class="apd-field"><span class="apd-label">ปี</span><span class="apd-value">${esc(a.modelYear||'-')}</span></div><div class="apd-field"><span class="apd-label">เลขไมล์ล่าสุด</span><span class="apd-value">${esc(a.mileage===''?'-':a.mileage)} ${esc(a.meterUnit||'')}</span></div><div class="apd-field"><span class="apd-label">บริษัท</span><span class="apd-value">${esc(a.companyName||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">หน่วยดูแล</span><span class="apd-value">${esc(a.unitName||'-')}</span></div></div></div></section>
    ${(s.attachments||[]).length?`<section class="apd-card"><div class="apd-card-head"><span class="apd-icon">🖼️</span><span>รูปประกอบอาการ</span></div><div class="apd-card-body apd-photos">${(s.attachments||[]).map(ph=>`<div class="apd-photo-item"><img src="${esc(ph.data)}" alt="${esc(ph.name||'รูป')}"><small>${esc(ph.name||'รูป')}</small></div>`).join('')}</div></section>`:''}
