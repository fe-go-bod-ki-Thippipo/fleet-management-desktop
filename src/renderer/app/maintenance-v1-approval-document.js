...
(function(){
  ...
  function approvalPanelsHtml(requestId){
    ...
    const versions=docs.length?`<table><thead><tr><th>Version</th><th>เลขที่เอกสาร</th><th>วันที่สร้าง</th><th>สถานะ</th><th></th></tr></thead><tbody>${docs.map(d=>`<tr><td>v${d.version}</td><td>${esc(d.documentNo)}</td><td>${esc(d.generatedAt||'-')}</td><td>${d.superseded?'<span class="pill">ถูกแทนที่แล้ว</span>':'<span class="pill ok">ฉบับล่าสุด</span>'}</td><td>${manage?`<button class="btn sm" data-apd-reprint="${esc(d.id)}">พิมพ์ซ้ำ</button>`:''}</td></tr>`).join('')}</tbody></table>`:'<div class="empty">ยังไม่มีเอกสารที่สร้าง</div>';
