...
(function(){
  ...
  function buildSnapshot(request){
    ...
    return freezeDeep({
      ...
      attachments:(request.attachments||[]).filter(x=>x&&String(x.type||'').startsWith('image/')&&x.data).slice(0,4).map(x=>({name:x.name||'รูป',data:x.data}))
    });
  }
  ...
  function printSheetHtml(doc){
    const s=doc.snapshotData,r=s.request,a=s.asset,p=s.proposal,urgency=urgencyPrintMeta(r.urgency);
    ...
    return `<div class="print-sheet apd-sheet"><style>
      ...
      .apd-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 18px}.apd-field{display:grid;grid-template-columns:120px 1fr;gap:8px;border-bottom:1px solid #e4eaf0;padding:5px 0;min-height:30px}.apd-field.apd-wide{grid-column:1/-1}.apd-label{font-size:13px;font-weight:700;color:#5b6f82}.apd-value{font-size:12px;color:#17283a;overflow-wrap:anywhere}
      ...
      .apd-asset-wrap{display:grid;grid-template-columns:210px 1fr;gap:16px;align-items:start}.apd-merged-wrap{display:grid;grid-template-columns:110px 1fr;gap:14px;align-items:start}.apd-asset-photo,.apd-photo-placeholder{width:210px;height:130px;border-radius:8px;border:1px solid #ccd8e3;background:#f4f7fa}.apd-merged-wrap .apd-asset-photo,.apd-merged-wrap .apd-photo-placeholder{width:110px;height:82px}.apd-asset-photo{display:block;object-fit:cover}.apd-photo-placeholder{display:flex;flex-direction:column;align-items:center;justify-content:center;color:#7b8c9d}.apd-photo-icon{font-size:34px;margin-bottom:3px}.apd-merged-wrap .apd-photo-icon{font-size:22px}.apd-photo-placeholder span{font-size:11px}
      ...
      .apd-approval{display:grid;grid-template-columns:1.08fr .92fr;gap:18px}.apd-choices{display:flex;flex-wrap:wrap;gap:10px;padding:5px 0}.apd-choice{display:inline-block;border:1px solid #cfd9e3;border-radius:7px;padding:7px 10px;background:#fbfcfd;font-weight:700;font-size:12px;white-space:nowrap}.apd-lines{font-size:12px;line-height:2.15}.apd-signature{border-left:1px solid #d9e2ea;padding-left:18px;min-height:120px;display:flex;flex-direction:column;justify-content:flex-start;text-align:center;font-size:12px}.apd-signature-bottom{justify-content:flex-end}.apd-sign-line-top{border-bottom:1px solid #66788a;height:38px;margin-bottom:6px}.apd-signature>div:nth-child(3),.apd-signature>div:nth-child(4){margin-top:8px;text-align:left}
      ...
      @media print{.apd-sheet{padding:10mm 9mm;font-size:11px}.apd-card{break-inside:avoid}.apd-header{grid-template-columns:1fr 255px}.apd-asset-wrap{grid-template-columns:180px 1fr}.apd-asset-photo,.apd-photo-placeholder{width:180px;height:112px}.apd-merged-wrap{grid-template-columns:90px 1fr}.apd-merged-wrap .apd-asset-photo,.apd-merged-wrap .apd-photo-placeholder{width:90px;height:67px}}
    </style><div class="apd-header"><div><div class="apd-brand"><div class="apd-logo">🚘</div><div><div class="apd-brand-name">${esc(company)}</div><div class="apd-brand-sub">FLEET MANAGEMENT · MAINTENANCE CONTROL</div></div></div><h2 class="apd-title">ใบขออนุมัติซ่อม</h2><div class="apd-subtitle">MAINTENANCE REPAIR APPROVAL REQUEST</div></div><div class="apd-meta"><div class="apd-meta-row"><b>เลขที่เอกสาร</b><span>${esc(doc.documentNo)}</span></div><div class="apd-meta-row"><b>วันที่ขอ</b><span>${esc(r.requestDate||'-')}</span></div><div class="apd-meta-row"><b>วันที่พิมพ์</b><span>${esc(String(doc.generatedAt||'').slice(0,10))}</span></div><div class="apd-meta-row"><b>สถานะ</b><span><span class="apd-status">${esc(statusText)}</span></span></div><div class="apd-meta-row"><b>ความเร่งด่วน</b><span><span class="apd-urgency ${urgency.cls}">${esc(urgency.label)}</span></span></div></div></div>
    <section class="apd-card"><div class="apd-card-head"><span class="apd-icon">📄</span><span>คำขอ / ทรัพย์สิน</span></div><div class="apd-card-body apd-merged-wrap">${assetPhoto}<div class="apd-grid"><div class="apd-field"><span class="apd-label">ผู้แจ้ง</span><span class="apd-value">${esc(r.requesterNameSnapshot||'-')}</span></div><div class="apd-field"><span class="apd-label">หน่วยงาน</span><span class="apd-value">${esc(a.unitName||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">อาการ / เหตุผล</span><span class="apd-value">${esc(r.issue||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">หมายเหตุ</span><span class="apd-value">${esc(r.requestNote||'-')}</span></div><div class="apd-field"><span class="apd-label">รหัสทรัพย์สิน</span><span class="apd-value">${esc(a.code||'-')}</span></div><div class="apd-field"><span class="apd-label">ทะเบียน / ชื่อ</span><span class="apd-value"><b>${esc(a.plate||'-')}</b></span></div><div class="apd-field"><span class="apd-label">ยี่ห้อ / รุ่น</span><span class="apd-value">${esc(`${a.brandName||''} ${a.modelName||''}`.trim()||'-')}</span></div><div class="apd-field"><span class="apd-label">ปี</span><span class="apd-value">${esc(a.modelYear||'-')}</span></div><div class="apd-field"><span class="apd-label">เลขไมล์ล่าสุด</span><span class="apd-value">${esc(a.mileage===''?'-':a.mileage)} ${esc(a.meterUnit||'')}</span></div><div class="apd-field"><span class="apd-label">บริษัท</span><span class="apd-value">${esc(a.companyName||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">หน่วยดูแล</span><span class="apd-value">${esc(a.unitName||'-')}</span></div></div></div></section>
    <section class="apd-card"><div class="apd-card-head"><span class="apd-icon">🔧</span><span>ข้อเสนอซ่อม</span></div><div class="apd-card-body apd-approval"><div class="apd-grid"><div class="apd-field apd-wide"><span class="apd-label">คำอธิบาย</span><span class="apd-value">${esc(r.issue||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">ผู้ให้บริการที่เสนอ</span><span class="apd-value">${esc(p.vendorNameSnapshot||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">ติดต่อ</span><span class="apd-value">${esc(p.vendorContact||'-')}</span></div><div class="apd-field apd-wide"><span class="apd-label">ประมาณการ</span><span class="apd-value"><b>${money(p.estimatedCost)} บาท</b></span></div></div><div class="apd-signature apd-signature-bottom"><div class="apd-sign-line-top">&nbsp;</div><div>หัวหน้าแผนกยานยนต์</div><div>วันที่ _________________________________</div></div></div></section>
    <section class="apd-card"><div class="apd-card-head"><span class="apd-icon">☑</span><span>ผลอนุมัติภายนอก</span></div><div class="apd-card-body apd-approval"><div><div class="apd-choices"><span class="apd-choice">□ อนุมัติ</span><span class="apd-choice">□ ไม่อนุมัติ</span><span class="apd-choice">□ อนุมัติแบบมีเงื่อนไข</span></div><div class="apd-lines">วงเงินที่อนุมัติ ______________________________ บาท<br>หมายเหตุ ____________________________________________</div></div><div class="apd-signature"><div class="apd-sign-line-top">&nbsp;</div><div>ลายเซ็นผู้อนุมัติ</div><div>ผู้อนุมัติ ______________________________</div><div>วันที่ _________________________________</div></div></div></section>
