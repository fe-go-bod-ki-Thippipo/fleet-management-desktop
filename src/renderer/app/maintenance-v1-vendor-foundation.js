...
(function(){
  ...
  const canManageVendor=()=>['admin','manager','fleetOfficer'].includes(role());
  ...
  function vendorPage(){
    ensureVendorState();setHead('ผู้ให้บริการ / อู่ซ่อม','Maintenance Vendor Master');const manage=canManageVendor();
    ...
    const draw=()=>{const q=norm($('#vendorQ')?.value),type=$('#vendorType')?.value||'',state=$('#vendorState')?.value||'active';let rows=STATE.vendors.filter(v=>!v.deleted);if(type)rows=rows.filter(v=>v.type===type);if(state==='active')rows=rows.filter(v=>v.active!==false);else if(state==='inactive')rows=rows.filter(v=>v.active===false);if(q)rows=rows.filter(v=>norm(`${v.code} ${v.name} ${v.contactName} ${v.phone} ${v.address}`).includes(q));$('#vendorRows').innerHTML=rows.length?`<table><thead><tr><th>รหัส</th><th>ชื่อผู้ให้บริการ</th><th>ประเภท</th><th>ผู้ติดต่อ</th><th>โทรศัพท์</th><th>สถานะ</th>${manage?'<th>จัดการ</th>':''}</tr></thead><tbody>${rows.map(v=>`<tr class="vendor-row" data-vendor-id="${esc(v.id)}"><td>${esc(v.code||v.id)}</td><td><b>${esc(v.name)}</b><div class="muted">${esc(v.address||'')}</div></td><td>${esc(v.type==='internal'?'หน่วยซ่อมภายใน':'อู่/ศูนย์ภายนอก')}</td><td>${esc(v.contactName||'-')}</td><td>${esc(v.phone||'-')}</td><td>${pill(v.active===false?'inactive':'active')}</td>${manage?`<td><button class="btn sm" data-vendor-edit="${esc(v.id)}">แก้ไข</button>${canDeleteVendor()?` <button class="btn sm" data-vendor-delete="${esc(v.id)}">ลบ</button>`:''}</td>`:''}</tr>`).join('')}</tbody></table>`:'<div class="empty">ยังไม่มีข้อมูลผู้ให้บริการ</div>';$$('[data-vendor-id]').forEach(r=>r.onclick=()=>vendorDetail(r.dataset.vendorId));$$('[data-vendor-edit]').forEach(b=>b.onclick=e=>{e.stopPropagation();vendorForm(b.dataset.vendorEdit)});$$('[data-vendor-delete]').forEach(b=>b.onclick=e=>{e.stopPropagation();deleteVendor(b.dataset.vendorDelete)});};
    $('#vendorQ').oninput=draw;$('#vendorType').onchange=draw;$('#vendorState').onchange=draw;$('#vendorReset').onclick=()=>{$('#vendorQ').value='';$('#vendorType').value='';$('#vendorState').value='active';draw()};if($('#newVendor'))$('#newVendor').onclick=()=>vendorForm();draw();
  }

  function vendorDetail(id){ensureVendorState();const v=STATE.vendors.find(x=>x.id===id);if(!v)return vendorPage();setHead(v.name,'Vendor Detail');const manage=canManageVendor()&&!v.deleted;content.innerHTML=`<div class="panel"><div class="toolbar"><button class="btn" id="vendorBack">← กลับทะเบียน</button><div>${manage?'<button class="btn primary" id="vendorEdit">แก้ไข</button>':''}</div></div>${v.deleted?'<div class="notice-row"><b>ข้อมูลประวัติ</b><span>ผู้ให้บริการนี้ถูก Soft Delete แล้ว และแสดงเพื่ออ้างอิงประวัติเท่านั้น</span></div>':''}<div class="detail-grid"><div><span class="muted">รหัส</span><b>${esc(v.code||v.id)}</b></div><div><span class="muted">ประเภท</span><b>${esc(v.type==='internal'?'หน่วยซ่อมภายใน':'อู่/ศูนย์ภายนอก')}</b></div><div><span class="muted">สถานะ</span><b>${esc(v.deleted?'ลบแล้ว':v.active===false?'ระงับใช้งาน':'ใช้งาน')}</b></div><div><span class="muted">ผู้ติดต่อ</span><b>${esc(v.contactName||'-')}</b></div><div><span class="muted">โทรศัพท์</span><b>${esc(v.phone||'-')}</b></div><div><span class="muted">แหล่งข้อมูล</span><b>${esc(v.source||'manual')}</b></div></div><div style="margin-top:16px"><span class="muted">ที่อยู่</span><p>${esc(v.address||'-')}</p><span class="muted">หมายเหตุ</span><p>${esc(v.note||'-')}</p></div></div>`;$('#vendorBack').onclick=vendorPage;if($('#vendorEdit'))$('#vendorEdit').onclick=()=>vendorForm(v.id);}

  function audit(action,id,before,after){if(typeof pAudit==='function')pAudit(action,'vendor',id,before,after);else save(true,action,'vendor',id,before,after);}
  function vendorForm(id=''){
    if(!canManageVendor())return toast('บทบาทนี้ไม่มีสิทธิ์แก้ไข Vendor Master',true);ensureVendorState();const old=id?STATE.vendors.find(x=>x.id===id&&!x.deleted):null;if(id&&!old)return toast('ไม่พบผู้ให้บริการ',true);
    formModal(old?'แก้ไขผู้ให้บริการ':'เพิ่มผู้ให้บริการ',`${fld('รหัส','code',old?.code||nextVendorCode(),true)}${fld('ชื่อผู้ให้บริการ / อู่','name',old?.name||'',true)}${sel('ประเภท','type',[['external','อู่/ศูนย์ภายนอก'],['internal','หน่วยซ่อมภายใน']],old?.type||'external')}${fld('ผู้ติดต่อ','contactName',old?.contactName||'')}${fld('โทรศัพท์','phone',old?.phone||'')}${area('ที่อยู่','address',old?.address||'')}${area('หมายเหตุ','note',old?.note||'')}${sel('สถานะ','active',[['true','ใช้งาน'],['false','ระงับใช้งาน']],String(old?.active!==false))}`,p=>{p.code=String(p.code||'').trim();p.name=String(p.name||'').trim();if(!p.code||!p.name)throw Error('กรุณากรอกรหัสและชื่อผู้ให้บริการ');if(vendorCodeDuplicate(p.code,id))throw Error('รหัสผู้ให้บริการซ้ำ');if(vendorDuplicate(p.name,id))throw Error('ชื่อผู้ให้บริการซ้ำ');p.active=p.active==='true';if(old){const before=structuredClone(old);Object.assign(old,p,{updatedAt:now()});audit('แก้ไขผู้ให้บริการ',old.id,before,old)}else{const x={id:uid('VEN'),...p,source:'manual',deleted:false,createdAt:now(),updatedAt:now()};STATE.vendors.push(x);audit('เพิ่มผู้ให้บริการ',x.id,null,x)}});
  }
