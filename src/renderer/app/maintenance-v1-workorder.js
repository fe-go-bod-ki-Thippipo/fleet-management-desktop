...
(function(){
  ...
  function saveOverBudgetNote(workOrderId,note){if(!canManage())throw Error('บทบาทนี้ไม่มีสิทธิ์แก้ไขหมายเหตุเกินวงเงิน');const wo=workOrderById(workOrderId);if(!wo)throw Error('ไม่พบ Work Order');const before=clone(wo);wo.overBudgetNote=String(note||'').trim();wo.updatedAt=ts();audit('บันทึกหมายเหตุเกินวงเงิน',wo.id,before,clone(wo));return wo;}
  ...
  function costPanelHtml(wo,cancelled){
    const parts=partsFor(wo.id),labours=labourFor(wo.id),external=externalCostsFor(wo.id),editable=canManage()&&!cancelled,noteEditable=canManage();
    ...
    const budget=isOverBudget(wo.id)?`<div style="margin-top:14px"><span class="pill danger">เกินวงเงินอนุมัติ</span><div class="muted">วงเงินอนุมัติ ฿${money(wo.approvedAmount)} · เกิน ฿${money(total-num(wo.approvedAmount))}</div><div class="toolbar" style="margin-top:8px"><div><b>หมายเหตุเกินวงเงิน</b><br><span>${esc(wo.overBudgetNote||'ยังไม่มีหมายเหตุ')}</span></div>${noteEditable?'<button class="btn sm" id="woEditOverBudgetNote">แก้ไขหมายเหตุ</button>':''}</div></div>`:'';
  ...
  function bindDetail(wo){
    ...
    if($('#woEditOverBudgetNote'))$('#woEditOverBudgetNote').onclick=()=>formModal('แก้ไขหมายเหตุเกินวงเงิน',`<label class="wide">หมายเหตุเกินวงเงิน<textarea name="note">${esc(wo.overBudgetNote||'')}</textarea></label>`,p=>{const x=saveOverBudgetNote(wo.id,p.note);setTimeout(()=>workOrderDetail(wo.id),0);return x});
