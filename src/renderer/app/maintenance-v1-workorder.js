...
(function(){
  ...
  function costPanelHtml(wo,cancelled){
    ...
    const budget=isOverBudget(wo.id)?`<div style="margin-top:14px"><span class="pill danger">เกินวงเงินอนุมัติ</span><div class="muted">วงเงินอนุมัติ ฿${money(wo.approvedAmount)} · เกิน ฿${money(total-num(wo.approvedAmount))}</div>${noteEditable?`<label class="wide" style="margin-top:8px">หมายเหตุเกินวงเงิน<textarea id="woOverBudgetNote">${esc(wo.overBudgetNote||'')}</textarea></label><button class="btn sm" id="woSaveOverBudgetNote">บันทึกหมายเหตุ</button>`:`<div>${esc(wo.overBudgetNote||'-')}</div>`}</div>`:'';
